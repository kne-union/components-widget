...
export const globalInit = async () => {
  ...
  const componentsCoreRemote = {
    ...registry, remote: 'components-core', defaultVersion: '0.1.76'
  };
  remoteLoaderPreset({
    remotes: {
      ...
      }, 'components-widget': process.env.NODE_ENV === 'development' ? {
        remote: 'components-widget', url: '/', tpl: '{{url}}', defaultVersion: process.env.DEFAULT_VERSION
      } : {
        ...registry, remote: 'components-widget', defaultVersion: process.env.DEFAULT_VERSION
      }
    }
  });

  const { default: getApis } = await loadModule('components-widget:AliSpeech@getApis');
